...
function isArrow(data: string, dir: string): boolean { return data === dir; }
...
function isAutocompleteTrigger(data: string, _line: string, col: number): boolean {
  if (data === "\t") return true;
...
export const customEditorWidgetFactory: WidgetFactory = (
  ...
  const gridRef = (deps as { gridRef?: { setText: (t: string) => void; getText: () => string } }).gridRef;
    ...
    if (result && result.items.length > 0) {
      ...
      completionEngine!.onApply = (selectedValue: string) => {
        ...
        const newState = autocompleteProvider.applyCompletion(
          state.lines, state.cursorLine, state.cursorCol, selected, prefix,
        );
  ...
  const instance: WidgetInstance = {
    render(width: number, height: number): string[] {
      ...
      const visibleLines = state.lines.slice(state.scrollOffset, state.scrollOffset + maxVisible);
    ...
    handleInput(data: string): boolean {
      ...
      if (data === "\r" || data === "\n") {
        if (state.lines.length > 1) { insertNewline(); return true; }
        submitText();
        return false; // let GridComponent.super.handleInput("\r") submit
      }

      if (isAutocompleteTrigger(data, state.lines[state.cursorLine] ?? "", state.cursorCol)) {
        insertChar(data);
        void triggerAutocomplete();
        return true;
      }

      if (data === "\x1b") { state.lines = [""]; state.cursorLine = 0; state.cursorCol = 0; return true; }
      ...
      if (isArrow(data, Arrow.Up)) { if (state.cursorLine > 0) state.cursorLine--; return true; }
      if (isArrow(data, Arrow.Down)) { if (state.cursorLine < state.lines.length - 1) state.cursorLine++; return true; }

      if (data === "\x01") { state.cursorCol = 0; return true; }
      if (data === "\x05") { const ln2 = state.lines[state.cursorLine] ?? ""; state.cursorCol = ln2.length; return true; }

      if (data === "\x0b") { const ln3 = state.lines[state.cursorLine] ?? ""; state.lines[state.cursorLine] = ln3.slice(0, state.cursorCol); return true; }
      if (data === "\x15") { const ln4 = state.lines[state.cursorLine] ?? ""; state.lines[state.cursorLine] = ln4.slice(state.cursorCol); state.cursorCol = 0; return true; }

      if (isPrintable(data) && data !== "\r" && data !== "\n") { insertChar(data); return true; }
